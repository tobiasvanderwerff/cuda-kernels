...
int main(int argc, char *argv[]){
	...
	int simStep = 0, inputFromFile = 0, initSteps = 0,total_simulation_steps = 0;
	float simTime = 0;
	cellState **cellPtr;
	...
	for (line_counter=0;line_counter<IO_NETWORK_SIZE;line_counter++) {
		...
		for (i=0; i<IO_NETWORK_SIZE; i++) {
			...
			if (ALLTOALL == 0){
				fscanf(pConFile, "%f ", &temp_cond_value); // reading into a temporary float, because fscanf apparently doesn't read into mod_prec
...
//				}
	...
	printf("int: %d, dend: %d, soma: %d, axon: %d, cellstate: %d\n", sizeof(int), sizeof(dend), sizeof(soma), sizeof(axon),  sizeof(cellState));
	...
	t1 = get_timestamp();
    secs = (t1 - t0);// / 1000000;
    printf("Execution completed: %ld ms of brain time of %ld neurons in %ld simulation steps executed\n", SIMTIME, cellCount, simStep);	
 ...
 */
	...
	int j,  requested_neighbour; // i, k=0, cell_id, translated_cell_id,  my_requested_cell
